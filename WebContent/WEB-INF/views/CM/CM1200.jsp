...
<html>
							...
							<table class="table-write">
								...
								<tbody>
									...
									<tr>
										...
										<td>
											<div class="input-btn">
												<input type="text" id="pop01_txt01_USER_NUM" Placeholder="사번" class="wp100">
												<a href="javascript:void(0);" id="pop01_btn02_VERIFICATION" class="btn">중복확인</a>
											</div>
										</td>
										<th>5.직급</th>
										<td>
	...
	<script type="text/javascript">
		...
		const userId = document.querySelector("#pop01_txt01_USER_ID");
		userId.addEventListener("keydown", () => validationValue(userId, "userId"));

		const userNum = document.querySelector("#pop01_txt01_USER_NUM");
		userNum.addEventListener("keydown", () => validationValue(userNum, "userNum"));

		/**
		 * 유효성 검사
		 */
		function validationValue(target, type) {
			let check = /^[A-Za-z0-9]*$/;
			if (!check.test(target.value)) {
				toast("정보", "영어와 숫자만 입력 가능합니다.", "info");
				target.value = "";
			} else {
				if(type === "userNum")
		...
		function setGrid(){
			$('#table1').jqGrid({
				...
				colNames: langHead,
				colModel: [
					{ name: 'USER_ID'		, align: 'center'	, width: '12%'	, hidden: false },
					{ name: 'USER_NM'		, align: 'center'	, width: '10%'	, hidden: false },
					{ name: 'USER_NUM'		, align: 'center'	, width: '9%'	, hidden: false },
					{ name: 'PDEPT_CD'		, align: 'center'	, width: '0%'	, hidden: true  },
					{ name: 'PDEPT_NM'		, align: 'center'	, width: '11%'	, hidden: false },
					{ name: 'DEPT_CD'		, align: 'center'	, width: '0%'	, hidden: true  },
					{ name: 'DEPT_NM'		, align: 'center'	, width: '11%'	, hidden: false },
					...
					{ name: 'EMAIL'			, align: 'left'		, width: '11%'	, hidden: false },
					{ name: 'PHONE_NUM'		, align: 'left'		, width: '11%'	, hidden: false },
					{ name: 'USE_YN'		, align: 'left' 	, width: '5%'	, hidden: false },
					{ name: 'AUTH_TYPE_CD'	, align: 'left'		, width: '0%'	, hidden: true  },
					{ name: 'AUTH_TYPE_NAME', align: 'left'		, width: '6%'	, hidden: false },
					{ name: 'MEMO'			, align: 'left'		, width: '11%'	, hidden: false }
				],
				autowidth: false,
				shrinkToFit: false,
				rowNum : 5000,
			...
			$("#table2").jqGrid({
				...
				rowNum : 5000,
				ondblClickRow : function(rowid){
					let rowdata = $("#table2").getRowData(rowid);
		...
		function updateGridData(selRowData){
			...
			$("#pop01_txt01_USER_NUM").val(selRowData.USER_NUM);
 			...
 			$('#pop01_txt01_USER_NUM').attr("disabled", true);
		...
		function searchGridData(){
			let searchParam = {
				USER_ID	: $("#txt01_USER_ID").val().trim(),
				USER_NM : $("#txt01_USER_NM").val().trim()
			};
		...
		$("#pop01_btn01_VERIFICATION").on({
			click: function() {
				if($("#pop01_txt01_USER_ID").val().trim() == ''){
					toast("경고", "사용자ID를 입력해주세요.", "error");
					return false;
				}else{
					searchVerificationUserIdOrNum("USER_ID");
				}
			}
		});

		$("#pop01_btn02_VERIFICATION").on({
			click: function() {
				if($("#pop01_txt01_USER_NUM").val().trim() == ''){
					toast("경고", "사번을 입력해주세요.", "error");
					return false;
				}else{
					searchVerificationUserIdOrNum("USER_NUM");
				}
			}
		});
		...
		function searchVerificationUserIdOrNum(type){
			...
			if (type === "USER_ID") {
				...
				searchParam = {
					USER_NUM: $("#pop01_txt01_USER_NUM").val().trim()
				};
			}

			getAjaxJsonData("cm1200Verification", searchParam, "searchVerificationUserIdOrNumCallBack");
		};
		...
		function searchVerificationUserIdOrNumCallBack(data) {
			...
			if(checkAction == "C"){
				...
				} else if (!isNaN(data[0].USER_NUM)) {
					let check = parseInt(data[0].USER_NUM);

					if (check > 0){
						checkUserNum = false;
						toast("경고", "이미 존재하는 사번입니다.", "error");
						return false;
					}else{
						checkUserNum = true;
						toast("성공", "사용 가능한 사번입니다.", "success");
					}
				}
		...
		function openModalPopup(action){
			...
			let returnPopup = getLangCodePopup("CM1200_Pop1", 11, "viewForm1", "${LANG}");
		...
		function openModalPopup_Dept(){
			// 화면ID, 화면ID사이즈(ex. 6:CM1000 / 13:CM1000_Detail), 팝업ID, 다국어
			let returnPopup = getLangCodePopup("CM1200_Pop2", 11, "viewForm2", "${LANG}");
			let titlePop = returnPopup[0];
			let pop02_btn01_FINISH = returnPopup[1];
				...
				, buttons: [
					{
	                    ...
	                    click : function(){
							let rowid = $("#table2").getGridParam("selrow");
							if(rowid < 1){
		...
		function openModalPopup_Auth(){
			// 화면ID, 화면ID사이즈(ex. 6:CM1000 / 13:CM1000_Detail), 팝업ID, 다국어
			let returnPopup = getLangCodePopup("CM1200_Pop3", 11, "viewForm3", "${LANG}");
			let titlePop = returnPopup[0];
